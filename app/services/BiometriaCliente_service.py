...
class BiometriaCliente_Service:
    ...
    async def list_biometries(self, cedula_cli: str, page: int, size: int, filter: dict | None = None) -> List[BiometriaCliente]:
        ...
        if not cliente_db_id:
            raise NotFound_Exception(message="No existe un cliente con la cédula indicada.")
    ...
    async def create_biometry(self, id_usuario: int, biometria_in: BiometriaCliente_Create) -> BiometriaCliente:
        ...
        if not entre_db:
            raise NotFound_Exception(message="No se pudo encontrar un entrenador con el ID de usuario dado.")
        ...
        if not cliente_db_id:
            raise NotFound_Exception(message="No existe el cliente con la cédula indicada en el sistema.")
